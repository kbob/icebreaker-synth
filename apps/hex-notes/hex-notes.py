...
class Top(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += [
            ...
            # midi_decode.note_on_rdy
            # midi_decode.note_off_rdy
            # midi_decode.note_chan
            # midi_decode.note_key
            # midi_decode.note_vel
            pri.note_on_rdy.eq(midi_decode.note_on_rdy),
            pri.note_off_rdy.eq(midi_decode.note_off_rdy),
            pri.note_chan.eq(midi_decode.note_char),
            pri.note_key.eq(midi_decode.note_key),
            pri.note_vel.eq(midi_decode.note_vel),
            # pri.mono_gate
            # pri.mono_key
            # pri.mono_vel
            ones_segs.digit_in.eq(pri.note_key[:4]),
            tens_segs.digit_in.eq(pri.note_key[4:]),
